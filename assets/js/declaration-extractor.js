...
export function getCustomsPageScore(page) {
  ...
  if (/\bcustoms?\s+declaration\b/i.test(topText)) score += 1200; else if (/\bcustoms?\b/i.test(topText)) score += 650;
  if (/(?:اقرار|بيان).*جمرك|جمرك.*(?:رقم|بيان)/.test(topAr)) score += 1050; else if (/جمرك/.test(topAr)) score += 450;
...
export function extractDeclaration(page, sourceFile) {
  ...
  return { sourceFile, pageNumber:page.pageNumber, decNo:dec?.value || "",
    ...
    description:validateDescription(primaryDescription, secondaryDescription), value:validateValue(primaryValue, secondaryValue),
...
function findDescription(lines) {
  ...
  const valueLine=lines.slice(goodsIndex+1).find(l=>hasValueLabel(l.text)); const stopY=valueLine?.top??Infinity, out=[];
  const sameLine=cleanDescription(textBeforeDetails(lines[goodsIndex].text));
  if (sameLine && !hasGoodsLabel(sameLine)) out.push(sameLine);
  for (const line of lines.slice(goodsIndex+1)) {
    if (line.top>=stopY) break; const raw=normalizeText(line.text).trim(); if (!raw) continue;
    const before=textBeforeDetails(raw); if (before!==raw) { const text=cleanDescription(before); if (text) out.push(text); break; }
...
function pickBestMoney(text) {
  ...
  const values=(normalized.match(MONEY_RE)||[]).map(raw=>({ value:raw.replace(/,/g,""), decimal:/[.,]\d{2,3}$/.test(raw), digits:(raw.match(/\d/g)||[]).length })).filter(x=>x.digits>=2);
