...
import { createOcrWorker, recognizePage, recognizeRegion, recognizeRegionEnhanced, terminateOcrWorker } from "./ocr-service.js?v=1.0.7";
import { extractDeclaration, getCustomsPageScore } from "./declaration-extractor.js?v=1.0.7";
import { getFieldRegions } from "./field-regions.js?v=1.0.7";
import { exportDeclarationExcel } from "./excel-export.js";
import { recognizeValueDecimalLayers, hasTwoDecimalValue } from "./value-ocr.js?v=1.0.7";
...
function acceptFiles(fileList) {
  ...
  setProgress(0, "Ready", "Each PDF will use a fresh isolated OCR worker."); setBusy(false, true, false);
}
...
async function scan() {
  ...
  try {
    ...
    for (let f=0; f<state.files.length && !state.stopRequested; f++) {
      await scanFileIsolated(state.files[f], f, dpi, includeImages);
      renderResults(state);
    }
...
async function scanFileIsolated(file, fileIndex, dpi, includeImages) {
  await terminateOcrWorker();
  setProgress((fileIndex/state.files.length)*100, `Preparing ${file.name}`, "Starting a fresh OCR worker for this PDF...");
  await createOcrWorker(m => handleOcrProgress(m));
  try { await scanFile(file, fileIndex, dpi, includeImages); }
  finally { await terminateOcrWorker(); }
}
...
async function scanFile(file, fileIndex, dpi, includeImages) {
  ...
  const loaded = await loadPdf(file), filePages = []; let best = null;
  try {
    for (let pageNo=1; pageNo<=loaded.pdf.numPages && !state.stopRequested; pageNo++) {
      ...
      try {
        ...
        filePages.push(page); const score=getCustomsPageScore(page); if (!best || score>best.score) best={page,score};
...
async function refineAndStore(page, pdf, fileName, dpi) {
  ...
  try {
    const header=await recognizeRegion(rendered.canvas,rendered.effectiveDpi,{left:0,top:0,width:rendered.canvas.width,height:rendered.canvas.height*0.34},"11");
    ...
    if (!/\d{2,}/.test(value.text)) value=await recognizeRegionEnhanced(rendered.canvas,rendered.effectiveDpi,valueRegion,{pageSegMode:"6",scale:2.1,threshold:null,contrast:1.8,whitelist:"0123456789.,$ "});
...
function scaleRegion(region,page,canvas) { const sx=canvas.width/(page.renderWidth||canvas.width), sy=canvas.height/(page.renderHeight||canvas.height); return { left:region.left*sx, top:region.top*sy, width:region.width*sx, height:region.height*sy }; }
function buildPage(fileName,pageNo,data,rendered,ocr) { return { sourceFile:fileName,pageNumber:pageNo,widthPt:data.widthPt,heightPt:data.heightPt,rotation:data.rotation,renderWidth:rendered.canvas.width,renderHeight:rendered.canvas.height,effectiveDpi:rendered.effectiveDpi,nativeText:data.nativeText,nativeItems:data.nativeItems,annotations:data.annotations,operationCount:data.operationCount,ocrText:ocr.text,ocrWords:ocr.words,ocrLines:ocr.lines,ocrConfidence:ocr.confidence,pageImageBlob:data.pageImageBlob||null }; }
